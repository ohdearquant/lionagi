...
| 6 | Validate that an operation response's `context` is a mapping before deep merge; on mismatch, produce one explicit failed status/result and add a regression test for progress and streaming status. | S | #2023 |
