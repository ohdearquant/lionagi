...
| 23 | Large ASAP graphs retain dependency order without millions of Dagre dummy nodes. | Exact large `minlen` values expanded a 111-node fixture to about 1,910 dummy nodes. | Costly 100+ node graphs apply a bounded gap budget; the reconstructed fixture stays within 222 and the measured layout is under 200 ms. |
...
| 25 | Schedule polling never starts a second refresh while one is active. | Timer, manual, and mutation refreshes could overlap the entire request fan-out. | `useSchedulesData` now shares one in-flight promise; fake-timer tests assert coalescing. |
