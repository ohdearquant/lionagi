...
# ---------------------------------------------------------------------------
# Edge cases: Exchange concurrent / ordering / unregister-mid-delivery
# ---------------------------------------------------------------------------


class TestExchangeEdgeCasesAdditional:
    @pytest.mark.anyio
    async def test_concurrent_collect_from_multiple_owners(self):
        import asyncio
        ...
        exchange.send(sender1, recipient, content={"n": 1})
        exchange.send(sender2, recipient, content={"n": 2})

        counts = await asyncio.gather(
            exchange.collect(sender1),
            exchange.collect(sender2),
        )
        assert sum(counts) == 2
        mail = exchange.receive(recipient)
        assert len(mail) == 2
    ...
    @pytest.mark.anyio
    async def test_message_delivery_fifo_ordering(self):
        exchange = Exchange()
        sender_id = uuid4()
        recipient_id = uuid4()
        ...
        for i in range(5):
            exchange.send(sender_id, recipient_id, content={"seq": i})

        await exchange.collect(sender_id)

        popped = []
        while True:
            m = exchange.pop_message(recipient_id, sender_id)
            if m is None:
                break
            popped.append(m.content["seq"])

        assert popped == list(range(5))

    @pytest.mark.anyio
    async def test_unregister_before_collect_drops_messages(self):
        exchange = Exchange()
        sender_id = uuid4()
        recipient_id = uuid4()
        ...
        exchange.register(sender_id)
        exchange.register(recipient_id)
        exchange.send(sender_id, recipient_id, content={"x": 1})

        exchange.unregister(recipient_id)

        count = await exchange.collect(sender_id)
        assert count == 0

    @pytest.mark.anyio
    async def test_broadcast_to_many_recipients(self):
        exchange = Exchange()
        sender_id = uuid4()
        recipients = [uuid4() for _ in range(20)]

        exchange.register(sender_id)
        for r in recipients:
            exchange.register(r)

        exchange.send(sender_id, None, content={"broadcast": True})
        count = await exchange.collect(sender_id)

        assert count == 1
        for r in recipients:
            mail = exchange.receive(r, sender=sender_id)
            assert len(mail) == 1

    @pytest.mark.anyio
    async def test_send_then_unregister_sender_before_collect(self):
        exchange = Exchange()
        sender_id = uuid4()
        recipient_id = uuid4()
        ...
        exchange.register(sender_id)
        exchange.register(recipient_id)
        exchange.send(sender_id, recipient_id, content={"orphan": True})

        exchange.unregister(sender_id)

        with pytest.raises(ValueError, match="not registered"):
            await exchange.collect(sender_id)

    @pytest.mark.anyio
    async def test_collect_all_tolerates_unregistered_during_iteration(self):
        exchange = Exchange()
        sender1 = uuid4()
        sender2 = uuid4()
