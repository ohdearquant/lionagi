...
async def _stub_run_dependencies(
    ...
    """Feed list_runs its rows without a store.

    The stub goes on list_sessions_page because that is what the run list
    calls. list_sessions is a thin wrapper over the same page read and
    patching it leaves the real read on the path, which returns nothing
    against an empty store and makes these contracts pass over zero rows.
    """
    import lionagi.studio.services.run_tags as run_tags
    import lionagi.studio.services.runs as runs_svc

    async def list_sessions_page(**_kwargs: Any) -> Any:
        return runs_svc._sessions_svc.SessionPage(
            items=[dict(row) for row in rows], next_cursor=None, has_more=False
        )

    async def tags_for_sessions(_ids: list[str]) -> dict[str, list[str]]:
        return {}

    monkeypatch.setattr(runs_svc._sessions_svc, "list_sessions_page", list_sessions_page)
