...
async def _seed_sessions(db_path: Path, sessions: list[dict]) -> None:
    async with StateDB(db_path) as db:
        for s in sessions:
            ...
            for column in ("model", "provider", "total_cost_usd"):
                if column in s:
...
def test_the_listing_resolves_model_and_cost_rather_than_passing_the_column_through(
    tmp_path, monkeypatch
):
    """Two columns are not fit to show, and both listings must resolve them.

    A provider CLI stamps `<synthetic>` on its own bookkeeping turns, which is
    not a model name, and codex spend is derived from a pricing table known to
    be wrong, where a plausible dollar figure is worse than an honest unknown.
    The runs listing and the sessions listing project the same rows through
    different code, so passing the raw column through in either one makes the
    two surfaces disagree about the same run.
    """
    db_path = tmp_path / "state.db"
    now = time.time()
    synthetic_id, codex_id, plain_id = (str(uuid.uuid4()) for _ in range(3))
    _run(
        _seed_sessions(
            db_path,
            [
                {"id": synthetic_id, "started_at": now - 30},
                {"id": codex_id, "started_at": now - 20},
                {"id": plain_id, "started_at": now - 10},
            ],
        )
    )

    async def _stamp_provenance():
        # Cost is not a create_session column; it lands through the same update
        # path a real run's usage accounting uses.
        async with StateDB(db_path) as db:
            await db.update_session(synthetic_id, model="<synthetic>")
            await db.update_session(codex_id, provider="codex", total_cost_usd=4.25)
            await db.update_session(plain_id, model="gpt-5", provider="openai", total_cost_usd=1.5)

    _run(_stamp_provenance())
    client = _make_client(tmp_path, monkeypatch, db_path)

    runs = {r["id"]: r for r in client.get("/api/runs", params={"per_page": 50}).json()["runs"]}
    listed = client.get("/api/sessions/", params={"limit": 50}).json()
    sessions = {s["id"]: s for s in (listed["sessions"] if isinstance(listed, dict) else listed)}

    for rows in (runs, sessions):
        assert rows[synthetic_id]["model"] is None
        assert rows[codex_id]["total_cost_usd"] is None
        # The resolutions are targeted, not a blanket blanking: a row carrying
        # a real model and a tracked provider keeps both.
        assert rows[plain_id]["model"] == "gpt-5"
        assert rows[plain_id]["total_cost_usd"] == 1.5
