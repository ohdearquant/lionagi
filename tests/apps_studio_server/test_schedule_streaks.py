...
async def test_list_schedules_issues_constant_queries_not_per_row(temp_db_path):
    ...
    # streak query, one batched health-evidence query. All four are per-call:
    # the count moves only when the path gains a query, never when it gains a
    # row.
    #
    # A writable open would add a fifth, reading the recorded schema version
