...
// The literal flag name `_apply_checkpoint_precompletion` (cli/orchestrate/flow.py)
// tells the operator to pass when a resume is refused for pending
// inherit_context ops — used here only to recognize that specific refusal
// shape among whatever else a detached resume can fail with.
const DEGRADED_CONTEXT_MARKER = "--allow-degraded-context";
const DEGRADED_POLL_INTERVAL_MS = 1500;
const DEGRADED_POLL_MAX_ATTEMPTS = 20;
...
function ResumeFlowRun({ runId, onResumed }: { runId: string; onResumed: Props["onResumed"] }) {
  ...
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<RunResumeResponse | null>(null);
  const [degradedRefusal, setDegradedRefusal] = useState<string | null>(null);
  ...
  function watchForDegradedRefusal(invocationId: string) {
    if (pollTokenRef.current) pollTokenRef.current.cancelled = true;
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (invocation.status === "failed") {
          const summary = invocation.status_reason_summary ?? "";
          if (summary.includes(DEGRADED_CONTEXT_MARKER)) {
            setDegradedRefusal(summary);
          }
          done = true;
        } else if (invocation.status !== "running" && invocation.status !== "queued") {
          done = true;
        }
      } catch {
        done = true;
      }
      if (!done && attempt < DEGRADED_POLL_MAX_ATTEMPTS && !token.cancelled) {
        window.setTimeout(() => void tick(), DEGRADED_POLL_INTERVAL_MS);
      }
    };
    void tick();
  }

  async function submit(allowDegradedContext: boolean) {
    setSubmitting(true);
    setError(null);
    if (!allowDegradedContext) {
      // A fresh (non-opt-in) request starts a new watch; an opt-in retry
      // keeps the prior refusal visible until its own outcome lands.
      setResult(null);
      setDegradedRefusal(null);
    }
    try {
      const accepted = await resumeRun(runId, { allow_degraded_context: allowDegradedContext });
      setResult(accepted);
      if (!allowDegradedContext) {
        watchForDegradedRefusal(accepted.invocation_id);
      } else {
        setDegradedRefusal(null);
      }
      await onResumed(accepted);
    } catch (caught) {
      // A failed submit keeps whatever refusal state was already visible —
      // it never gets silently cleared by a request that itself failed.
  ...
  return (
    <SectionShell runId={runId}>
      ...
      {degradedRefusal && (
        <div
          role="alert"
          className="mt-2 grid gap-2 rounded border border-status-failure/30 bg-status-failure-bg px-2.5 py-2 text-body text-status-failure"
        >
          <p className="whitespace-pre-wrap">{degradedRefusal}</p>
          <Button
            variant="secondary"
            size="sm"
            disabled={submitting}
