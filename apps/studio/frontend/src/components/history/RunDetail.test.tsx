...
// ─── runFiles comes only from the bounded server summary ─────────────────────
// Sessions are windowed, and raw tool arguments can contain absolute host
// paths or credentials. The full-run server summary is therefore the only
// eligible source for both the visible section and Markdown file links.

describe("history/RunDetail.tsx — runFiles uses session.run_files", () => {
  const src = fs.readFileSync(path.join(HISTORY_DIR, "RunDetail.tsx"), "utf-8");
