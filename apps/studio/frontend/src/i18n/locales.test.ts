...
describe("messages — leaf-key parity across all 16 locales", () => {
  ...
  //
  // 1106, measured from en.json rather than carried from either side: main
  ...
  // discard-changes confirmation, schedules.error.unclassified for a failure the
  // server could not classify, and schedules.table.historyUnavailable to tell a
  // run slice that failed to load apart from a schedule that never fired. All
  // seven additions are natively translated in all 16 locales, so they add
  // nothing to the identity-leak baseline.
  it("en.json has 1106 leaves", () => {
    expect(EN_LEAVES.size).toBe(1106);
  });
