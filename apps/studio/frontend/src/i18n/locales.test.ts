...
describe("messages — leaf-key parity across all 16 locales", () => {
  ...
  //
  // 1099 = 1098 + shared.activeSnapshotOmitted (1), the notice shown when an
  // active-work snapshot had to leave rows out.
  //
  // Both sides of this merge raised 1097 by one, for different leaves, so the
  // assertion below auto-merged to a number both agreed on while the catalog
  // had gained two. The count is re-derived from the merged en.json rather
  // than reasoned about.
