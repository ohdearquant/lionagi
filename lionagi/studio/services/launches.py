...
def _failure_reason_summary(status: str, output_tail: str) -> str:
    ...
    A failed `orchestrate flow --resume` names its offending ops in exactly
    this text (see FlowResumeError in _checkpoint.py / flow.py); discarding
    it here was the reason a degraded-context refusal surfaced to Studio as
    an unexplained "Detached launch failed." Kept short and tail-truncated —
    this lands in a DB column meant for a summary, not a full traceback.
    """
