...
def _build_nodes(session: dict[str, Any], run_id: str) -> list[dict[str, Any]]:
    ...
    branch_by_name = {b["name"]: b for b in branches if isinstance(b, dict) and b.get("name")}
...
def _build_stalls(nodes: list[dict[str, Any]], session: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for node in nodes:
        ...
        if segment is not None:
            last_activity = segment.get("last_heartbeat_at") or segment.get("started_at")
        if last_activity is None:
            last_activity = node.get("started_at")
        if last_activity is None:
            continue
        stalls.append(
            {
