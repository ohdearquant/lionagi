...
def _ruff_check_sync(paths: list[str], max_diagnostics: int) -> CodeCheckResponse:
    ...
    total = len(raw_diags)
    capped = total > max_diagnostics
    cap_note = f" (showing {max_diagnostics} of {total})" if capped else ""
    lead = ", ".join(d.as_text() for d in diagnostics[:3])
    trailer = f" ... (+{len(diagnostics) - 3} more)" if len(diagnostics) > 3 else ""
    summary = f"{len(diagnostics)} issue(s){cap_note}: {lead}{trailer}"
