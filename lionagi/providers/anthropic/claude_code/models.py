...
async def stream_claude_code_cli(  # noqa: C901
    ...
    *,
    ...
    # Verbose: render the session header exactly once per session_id. Claude Code
    # can emit several `system` events for one session; only the init carries
    # `model`/`tools`, so re-printing on every event produced duplicate blocks
    # that degraded to `model: ?` with empty tools. Dedupe + require `model`.
        ...
        async for obj in stream:
            ...
            if typ == "system":
                ...
                if request.verbose_output:
                    ...
                    if data.get("model") and sid not in seen_system_ids:
