...
def translate_permissions(policy: Any) -> dict[str, Any]:
    """Translate a ``PermissionPolicy`` into ``ClaudeCodeRequest`` field kwargs."""
    ...
    if mode == "deny_all":
        return {"disallowed_tools": list(all_tools)}

    allowed: list[str] = []
    denied: list[str] = []
    for zone, cc_names in PERMISSION_TOOL_MAP.items():
        patterns = getattr(policy, "allow", {}).get(zone, [])
        if patterns and ("*" in patterns or any(p.strip() == "*" for p in patterns)):
            allowed.extend(cc_names)
        if zone in getattr(policy, "deny", {}):
            denied.extend(cc_names)

    result: dict[str, Any] = {}
    if allowed:
        result["allowed_tools"] = list(dict.fromkeys(allowed))
    if denied:
        result["disallowed_tools"] = list(dict.fromkeys(denied))
    return result
