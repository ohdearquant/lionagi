...
def _do_match(p: str, v: str, f: int, q: multiprocessing.Queue) -> None:  # type: ignore[type-arg]
    """Module-level helper for pattern matching in a subprocess.

    Must be at module level (not a local or nested function) so it is
    picklable on all platforms including macOS ``spawn``-start-method.
    """
