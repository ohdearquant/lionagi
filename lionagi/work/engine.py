...
class WorkEngine:
    ...
    def submit(self, form: WorkForm, worker_id: str | None = None) -> str:
        ...
        Args:
            form: The WorkForm to process.  Must be in ``validated`` or
                ``filled`` status (the engine accepts both; workers receive
        ...
        Raises:
            ValueError: No workers registered, or *worker_id* not found.
    ...
    def _run_task(self, task: WorkTask, slot: _WorkerSlot, form: WorkForm) -> None:
        ...
        timeout = slot.definition.timeout_seconds
        ...
        try:
            if timeout:
                ...
                # SIGALRM is Unix-only; fall back to non-timeout on Windows.
